...
public class ExePath {
	...
	float[] runPath(float[] testData, int triang, FitnessFunction fit) {
		...
		for(Condition each: getBranches() ){
			testData = calculateTestDataByCondition(each, testData, tri, fit);
		}
		return testData;
		
	}
	float[] calculateTestDataByCondition(Condition condition, float[] testData, int triang, FitnessFunction fit) {
		int Total = 0;
